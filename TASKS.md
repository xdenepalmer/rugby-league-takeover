...
| AD-OPT | Ad system audit + optimization | completed (await merge) | `feat/ad-system-optimization` off `ac27704`. Real DB analytics (`recordAdEvent` fn + SiteAd `impression_count`/`click_count` + AdRevenueTracker), `ads_enabled` default→true, `device_target` wired, dead sidebar/in-feed positions removed. 45/45 green. 🔴 Base44 deploy (recordAdEvent + SiteAd + SiteSettings) required; A/B (ab_variants) deferred |
