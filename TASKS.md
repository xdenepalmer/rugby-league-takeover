...
| ADMIN-CC | Admin/public UX upgrade (epic, incremental) | in_progress | Step 1 (build green) **verified** — Home.jsx bugs not on main. Step 2 **done** (`feat/admin-command-center`): `AdminActionQueue` "Needs you now" task queue atop OverviewPanel (orders/posts/low-stock/registrations/drafts → /admin links), frontend-only, 45/45. Next: StoreOrder shipping model (deploy), moderation inbox, My-Takeover account, contextual mobile sheets, notification prefs |
