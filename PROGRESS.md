...
_Last updated: 2026-07-10 (RLT-IOS-003 + corrections 003F–003H)_

> **RLT-IOS-003F–H — Architect corrections (code complete, awaiting adversarial review).** Blocking finding 1 fixed: `createCheckout` now returns Stripe to `/store/checkout/success?session_id={CHECKOUT_SESSION_ID}` / `/store/checkout/cancel`; NEW read-only `verifyCheckoutReturn` edge fn verifies the session server-side (app-id + stored session-id double bind, PII-free result); the native return screen is a verification state machine — success copy + a single guarded exactly-once cart clear happen ONLY on verified payment, pending polls bounded and keeps the cart, unverified never claims success. ⚠️ Requires Supabase deploy of `createCheckout` (changed) + `verifyCheckoutReturn` (new); until deployed, production still uses the legacy web-banner return. Blocking finding 2 fixed: TRUE native list/detail workflows for Orders (`/admin/store/orders[/:orderId]` — filters, contact actions, tracking editor, AusPost label/track, confirmed transitions with payload parity), Forum moderation (queues, publish/hide/pin/remove/restore, Ban author/IP via BanDialog, open-fan-thread) and Registrations (list/detail, contact, bulk BCC, read-only); the other 19 modules remain wrapped web managers (functional parity). Status language corrected throughout (no "byte-preserved"; granular admin/push/checkout/TestFlight fields). Validation: **tests 202/202 · lint 0 · typecheck 0 · build 0 · cap sync clean**.

> **RLT-IOS-003 — native product transformation (code complete, awaiting Architect review).** Branch `rlt-ios-003-native-product` off `main` (`98f917d`), five story commits (audit `697797a`, 003A `b34eaac`, 003B `bf33b8a`, 003C `4d1409a`, 003D `1a0afb8`) + 003E docs/QA. The Capacitor shell now renders its OWN route tree: five-tab fan shell (Home dashboard / News + `/news/:id` reader / Forum + `/forum/thread/:id` / Store + `/store/product/:id` + cart sheet / Account hub), Takeover sheet (Plan, Gallery, FAQ, legal, accent picker, gated Admin entry), touch-first gallery viewer (swipe/pinch/drag-dismiss), five-tab native admin shell (Overview attention queue + all 22 managers URL-addressable), per-tab route + scroll memory, semantic throttled haptics, query-cache persistence with an admin-PII denylist, foreground-resume invalidation, windowed feeds, notification tap-routing, checkout return screens (webhook stays authority), canonical share/deep-link routes valid on BOTH platforms, `ios-build-verify` Codemagic workflow. Web/PWA presentation and behavior preserved and regression-tested (canonical detail routes added deliberately; preload list unchanged, native chunks lazy-only; smoke-verified SiteNav + Google button intact). isNativeApp() now latches (route tree can never flip mid-session). Validation at every commit: **tests 181/181 · lint 0 · typecheck 0 · build 0 · cap sync clean**; Chromium smoke (iPhone viewport + Capacitor stub) green for native Home/News/Forum/Store/Gallery/Login + 5-tab/no-admin-tab contract. Deliberately NOT done (documented in docs/NATIVE_ARCHITECTURE.md): APNs delivery (RLT-IOS-004), native Google OAuth (exact PKCE/universal-link plan written; Google stays hidden), Xcode/TestFlight validation (Apple signing still being finished by owner).
