...
_Last updated: 2026-07-07 (RLT-IOS-003)_

> **RLT-IOS-003 — first-build iOS prerequisites (code complete, awaiting merge).** Branch `claude/new-session-p5p9zs`. In-repo prep for the first Codemagic/TestFlight build (no Mac / no design masters needed). AGY provisioned the Apple side: App ID `com.rugbyleaguetakeover.app` registered with Push + Associated Domains, App Store Connect record created (SKU RLT-IOS-001), Team ID **25R438YK9F confirmed** (matches the AASA edit). Committed + wired into the Xcode project: `App.entitlements` (associated-domains `applinks:rugbyleaguetakeover.com`, `CODE_SIGN_ENTITLEMENTS` on Debug+Release); Info.plist camera/mic/photo usage strings; `PrivacyInfo.xcprivacy` (required-reason APIs CA92.1/C617.1/35F9.1) added to the Resources build phase; AASA `TEAMID`→`25R438YK9F`. Push entitlement (aps-environment) intentionally deferred to the push story. New `tests/ios-project-config.test.mjs` guards all of it. Validation: **tests 118/118 · lint 0 · typecheck 0 · build 0 · cap sync clean (plists/pbxproj preserved)**. NO secrets in repo (APNs key ZT4QQWTC56 + Codemagic API key HX3GX68S4S live in Codemagic/Supabase env only). Still pending: actual Codemagic build → TestFlight (owner, from Windows), 1024px icon/splash masters, push delivery, in-app auth/checkout return.
