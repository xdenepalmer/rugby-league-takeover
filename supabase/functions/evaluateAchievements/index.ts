...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // The database atomically inserts only not-yet-recorded unlocks and
    // increments the chip balance for those inserts in the same transaction.
    // The unique (user_id, achievement_id) index makes concurrent evaluations
    // safe and prevents double rewards.
