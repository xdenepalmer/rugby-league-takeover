...
import { createCheckoutRequestId } from "@/lib/store-checkout";
...
const formatDate = (timestamp) => timestamp
  ? new Date(Number(timestamp) * 1000).toLocaleString("en-AU", {
      day: "2-digit",
      month: "short",
      year: "numeric",
      hour: "2-digit",
...
export default function PromoCodesManager() {
  ...
  const promoQuery = useQuery({
    ...
    queryFn: async () => (await base44.functions.invoke("promoCodes", { action: "list" })).data,
    staleTime: 30_000,
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!Number.isFinite(value) || value <= 0 || (draft.discountType === "percent" && value > 100)) {
        throw new Error(draft.discountType === "percent"
          ? "Percentage must be between 0.01 and 100."
          : "Enter a valid fixed AUD discount.");
      }
      if (!Number.isFinite(minimum) || minimum < 0) throw new Error("Minimum spend cannot be negative.");
      if (limit !== null && (!Number.isInteger(limit) || limit < 1)) {
        throw new Error("Redemption limit must be a positive whole number.");
      }
      ...
      <div className="p-5 md:p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" size="mobile" onClick={() => setShowCreate((value) => !value)} className="rounded-none text-[9px] font-bold uppercase tracking-wider">
          ...
          <div className="mb-5 space-y-4 border border-primary/25 bg-primary/[0.03] p-4 md:p-5">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <label className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(event) => setDraft({ ...draft, code: event.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, "") })}
              ...
              <label className="space-y-1">
                ...
                <select value={draft.discountType} onChange={(event) => setDraft({ ...draft, discountType: event.target.value })} className="h-11 w-full rounded-none border border-border/50 bg-background px-3 text-sm text-foreground">
              ...
              <label className="space-y-1">
                ...
                <Input type="number" min="0.01" max={draft.discountType === "percent" ? "100" : "50000"} step="0.01" value={draft.discountValue} onChange={(event) => setDraft({ ...draft, discountValue: event.target.value })} className="h-11 rounded-none border-border/50" />
              </label>
              <label className="space-y-1">
                <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Minimum merch spend (AUD)</span>
                <Input type="number" min="0" step="0.01" placeholder="Optional" value={draft.minimumSubtotalAud} onChange={(event) => setDraft({ ...draft, minimumSubtotalAud: event.target.value })} className="h-11 rounded-none border-border/50" />
              </label>
              <label className="space-y-1">
                <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Maximum uses</span>
                <Input type="number" min="1" step="1" placeholder="Unlimited" value={draft.maxRedemptions} onChange={(event) => setDraft({ ...draft, maxRedemptions: event.target.value })} className="h-11 rounded-none border-border/50" />
              </label>
              <label className="space-y-1">
                <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Expires (Brisbane time)</span>
                <Input type="datetime-local" value={draft.expiresAt} onChange={(event) => setDraft({ ...draft, expiresAt: event.target.value })} className="h-11 rounded-none border-border/50" />
              </label>
            </div>
            <div className="flex flex-col justify-between gap-3 border-t border-border/40 pt-4 sm:flex-row sm:items-center">
              <p className="max-w-2xl text-[10px] leading-relaxed text-muted-foreground">
                Minimum spend and free shipping use the merchandise subtotal before discount. Stripe records successful redemptions.
              </p>
              <Button type="button" disabled={createMutation.isPending} onClick={() => createMutation.mutate()} className="min-h-[44px] shrink-0 rounded-none px-6 text-[10px] font-bold uppercase tracking-wider">
        ...
        {promoQuery.isError && (
          <div className="border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
            Promo codes could not be loaded. Confirm the function and Stripe keys are configured, then refresh.
          </div>
        )}
                ...
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`border px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider ${active ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-border/50 bg-muted/20 text-muted-foreground"}`}>
                        {active ? "Active" : exhausted ? "Used up" : expired ? "Expired" : "Inactive"}
                      </span>
                      <span className="border border-violet-500/25 bg-violet-500/10 px-2 py-0.5 text-[9px] font-bold text-violet-300">{describeDiscount(promotion)}</span>
                    </div>
                  ...
                  {active && (
                    <Button type="button" variant="outline" size="mobile" onClick={() => setDeactivateTarget(promotion)} className="shrink-0 rounded-none border-destructive/30 text-[9px] font-bold uppercase tracking-wider text-destructive hover:bg-destructive/10">
