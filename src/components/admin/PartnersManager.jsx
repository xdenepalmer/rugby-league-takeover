        ...
        {sorted.map((partner) => (
          <div key={partner.id} className="grid gap-3 border border-border p-4">
            ...
            <div className="flex items-center gap-4 border-t border-border pt-3">
              <label className="flex min-h-11 items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">Published <Switch checked={partner.is_published !== false} onCheckedChange={(v) => updateMutation.mutate({ id: partner.id, data: { is_published: v } })} /></label>
              <Button variant="destructive" size="mobile" className="ml-auto rounded-none" onClick={() => deleteMutation.mutate(partner.id)}><Trash2 className="mr-2 h-4 w-4" /> Delete</Button>
            </div>
          </div>
        ))}
      </div>
