...
function TestimonialCard({ t }) {
  return (
    <figure
      className="group relative flex h-full min-w-[320px] max-w-[380px] flex-col
                 border border-border cmd-glass rounded-none p-6
                 transition-all duration-300
                 hover:border-transparent"
    >
      {/* Gradient border on hover — rendered as an inset pseudo-layer */}
      <span
        aria-hidden="true"
      ...
      <div className="relative w-fit">
        <Quote className="h-7 w-7 text-primary/60 transition-all duration-300 group-hover:text-primary" />
        <span
          aria-hidden="true"
          className="absolute inset-0 blur-lg opacity-0 transition-opacity duration-300
                     group-hover:opacity-40"
          style={{ background: "hsl(var(--primary))" }}
        />
      </div>

      <blockquote className="mt-4 flex-1 text-sm leading-7 text-foreground/90">
        {t.quote}
      </blockquote>
      ...
      <figcaption className="mt-5 flex items-center gap-3 border-t border-border pt-4">
        ...
        <div className="min-w-0">
          <p className="truncate font-display text-sm uppercase text-foreground">
            {t.author_name}
          </p>
          {t.author_role && (
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="group/carousel relative mt-10 overflow-hidden">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to right, hsl(var(--background) / 1), hsl(var(--background) / 0))",
            }}
          />
          {/* Right gradient fade */}
          <div
            ...
            style={{
              background:
                "linear-gradient(to left, hsl(var(--background) / 1), hsl(var(--background) / 0))",
            }}
          />

          {/* Scrolling track — pauses on hover */}
          <div
            className="animate-marquee gap-5 py-1 group-hover/carousel:[animation-play-state:paused]"
          >
            {loopCards.map((t, index) => (
              <div key={`${t.id || index}-${index}`} className="shrink-0">
                <TestimonialCard t={t} />
              </div>
            ))}
          </div>
