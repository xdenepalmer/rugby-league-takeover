...
async function findActiveBan(base44, { ip, emails = [], userId }) {
  ...
  for (const candidate of candidates) {
    const matches = await base44.asServiceRole.entities.Ban.filter({
      ...candidate,
      is_active: true
    });
...
Deno.serve(async (req) => {
  try {
    ...
    if (!team) {
      return Response.json({ error: 'Team supported is required' }, { status: 400 });
    }
    ...
    const ban = await findActiveBan(base44, {
      ip,
    ...
    if (ban) {
      return Response.json(
        {
          error: 'Your account or connection has been blocked.',
          code: 'blocked'
        },
        { status: 403 }
      );
