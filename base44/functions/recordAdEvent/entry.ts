...
// (RLS blocks anonymous writes, so this runs with the service role). Self-contained
// by design — Base44 does not support cross-function imports.
//
...
Deno.serve(async (req) => {
  try {
    ...
    if (!id) return Response.json({ error: 'adId is required' }, { status: 400 });
    if (type !== 'impressions' && type !== 'clicks') {
      return Response.json({ error: 'type must be "impressions" or "clicks"' }, { status: 400 });
    }
    ...
    await base44.asServiceRole.entities.SiteAd.update(id, { [field]: next });

    return Response.json({ ok: true, [field]: next });
  } catch (error) {
