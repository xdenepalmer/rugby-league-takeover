...
## 2026-07-10 — RLT-IOS-003B (branch rlt-ios-003-native-product)
Native fan screens: Home dashboard (countdown/headline/trending-thread/matchup/event/merch/plan CTA, signed-in strip), News feed + /news/:id native reader (progress, text-size, bookmarks, bounded offline cache) + web NewsArticle page, Forum feed + /forum/thread/:id detail (reactions, replies, keyboard-safe composer, per-thread drafts, report, fan tools incl ScorePredictor/slots) + composer sheet, Store catalogue + /store/product/:id + cart sheet (AusPost rates + server-authoritative createCheckout handoff), touch-first Gallery grid + gesture viewer (swipe/pinch/drag-dismiss/prefetch), Account hub list/detail + notifications centre. Shared: use-fan-data/use-forum-actions hooks (same query keys as web), cart-store/store-products/news-articles libs, per-entity share URLs, web aliases (/forum/thread/:id→?thread=, /store/product/:id→?product=, /gallery?item=). Validation: tests 162/162, lint 0, typecheck 0, build 0, cap sync clean; web preloads unchanged (native chunks lazy-only).

## 2026-07-10 — RLT-IOS-003C (branch rlt-ios-003-native-product)
Native admin operations shell: five admin tabs (Overview/Content/Store/Community/More) replacing the desktop command-centre chrome on iOS; native Overview (stat tiles + needs-attention queue deep-linking into modules); section hubs -> full-screen module screens hosting ALL 22 existing managers with panel-identical data wiring (news/travel/gallery/faqs/partners/testimonials/products/orders/forum/users/registrations/invites/bans/events/teams/matchups/settings/ads/sponsors/calendar/revenue/export); sub-modules now URL-addressable (/admin/store/orders — native only); attention badges (pending posts/paid orders, 60s); More tab: Events/People/Ads/Settings/Export + Return-to-fan + Sign out. Web AdminLayout untouched. Validation: tests 170/170, lint 0, typecheck 0, build 0, cap sync clean.

## 2026-07-10 — RLT-IOS-003D (branch rlt-ios-003-native-product)
Native state/performance/return flows: query-cache persistence (native-only dynamic module; denylist for orders/registrations/users/bans/invites/adminAttention/myOrders; 24h maxAge; build-id buster; SIGNED_OUT clears; persist pkgs exempted from web vendor chunks — verified no preload leak), foreground-resume selective invalidation (notifications/forumPosts/adminAttention), session-persisted scroll memory, windowed rendering for forum/news feeds (IntersectionObserver, no new dep), notification-tap routing map (forum_reply/mention/news/product_drop/order_update/link with hostile-link fallback) wired via NativeRuntime, Stripe checkout return: native /store/checkout/success|cancel screens (webhook stays authority; closes browser sheet; success clears cart per web parity) + web alias routes + native /store?success redirect, More-sheet build stamp, codemagic ios-build-verify workflow (unsigned compile, no publishing). Native Google OAuth deliberately NOT enabled: safe PKCE/universal-link flow cannot be device-proven here — exact plan documented in 003E docs; Google stays hidden in shell. Validation: tests 181/181, lint 0, typecheck 0, build 0, cap sync clean.

## 2026-07-10 — RLT-IOS-003E (branch rlt-ios-003-native-product)
QA + docs + handoff: Chromium native-mode smoke (iPhone viewport, Capacitor stub) — native Home/News/Forum/Store/Gallery/Login all render the native shell; contract probe: exactly 5 fan tabs, no Admin tab, one Takeover trigger; web regression: SiteNav + Google button intact, no page errors. Smoke exposed + fixed a real fragility: isNativeApp()/getPlatform() now LATCH their first answer (the @capacitor/core web runtime replaces a pre-set window.Capacitor when the first plugin loads, which could flip the route tree mid-session). Docs: new docs/NATIVE_ARCHITECTURE.md (route/tab/admin maps, caching denylist, haptic vocabulary, checkout-return + push status, exact native-OAuth plan, known gaps); README/iOS_RUNBOOK/APP_STORE_CHECKLIST/PROGRESS/TASKS updated. Final gate: tests 181/181, lint 0, typecheck 0, build 0, cap sync clean. dist 7.2MB/160 chunks; entry ~71KB (baseline 69.5KB); all 14 native screens are lazy chunks, zero native preloads on web.

## 2026-07-10 — RLT-IOS-003F (branch rlt-ios-003-native-product)
Authoritative checkout return (Architect blocking finding 1): createCheckout now emits canonical return URLs with {CHECKOUT_SESSION_ID}; NEW read-only verifyCheckoutReturn edge fn (validates session-id format, retrieves session from Stripe server-side, rejects foreign rlt_app_id, double-binds order via stored stripe_session_id, returns PII-free {paymentStatus,sessionStatus,orderStatus}); native return screen is now a verification state machine (confirming→confirmed/pending/cancelled/unverified) — success copy + exactly-once cart clear ONLY on verified payment (per-session marker), pending polls bounded (5x4s) and keeps cart, unverified never claims success. Guest-safe: unguessable cs_ id is the return credential. Web keeps legacy banner flow via aliases. DEPLOY REQUIRED: createCheckout (changed) + verifyCheckoutReturn (new) to Supabase; no new secrets (reuses STRIPE key + RLT_APP_ID). Validation: tests 189/189, lint 0, typecheck 0, build 0, cap sync clean.

## 2026-07-10 — RLT-IOS-003G (branch rlt-ios-003-native-product)
Priority native admin workflows (Architect blocking finding 2): true native list/detail experiences replace the wrapped web managers for the three highest-value modules. Orders: searchable/filterable list (pipeline groups mirror web), URL-addressable detail (/admin/store/orders/:orderId) w/ customer contact (mail/tel/copy-address), line items, tracking editor + copy, AusPost label + tracking refresh (same edge fns), confirmed status transitions writing byte-identical payloads (timeline entries, shipped_at/delivered_at, tracking on shipped), cancel/refund destructive confirms, payment state labeled webhook-authoritative. Moderation: pending/reported/live/removed queues w/ counts, search, publish/hide/pin/remove(reason)/restore actions (exact web payloads incl moderation_reason + is_published:false on restore), Ban author/IP via the same BanDialog + Ban.create shape, open fan thread. Registrations: searchable list + detail (/admin/people/registrations/:regId), contact actions, bulk BCC email w/ mail-app cap warning, read-only (no mutations), PII stays persistence-denylisted. Classic managers retained as exported escape hatches; other 19 modules unchanged; web admin untouched. Validation: tests 202/202, lint 0, typecheck 0, build 0, cap sync clean.

## 2026-07-10 — RLT-IOS-003H (branch rlt-ios-003-native-product)
Handoff truth corrections: "byte-preserved" language replaced with "presentation and behaviour preserved and regression-tested" (App.jsx comment, PROGRESS, docs); NATIVE_ARCHITECTURE rewritten for the verified checkout-return flow, the delivered native admin workflows and a granular honest status matrix (admin shell/parity/priority-workflows/all-native split; checkout routes/verification/deployed/device-verified split; push code/migration/live/delivery split; codemagic/signed-ipa/testflight-upload/install split); APP_STORE_CHECKLIST + iOS_RUNBOOK gained the edge-fn deployment requirement; TASKS status → corrections landed, awaiting adversarial review. Final gate: tests 202/202, lint 0, typecheck 0, build 0, cap sync clean.

## 2026-07-10 — RLT-IOS-003I–003M (branch claude/rlt-ios-003-corrections-ozfpqg, corrections for rlt-ios-003-native-product)
Adversarial-review corrections, one bounded commit per story, gate green after each. 003I security/data: query persistence inverted denylist→ALLOWLIST (only public non-PII roots persist; mutations never dehydrate; the 003D denylist had been serializing admin forum projections w/ ip_address+user_email to localStorage), verifyCheckoutReturn made DB-first (store_orders lookup by stripe_session_id BEFORE any Stripe call → unknown ids 404 free = anti-amplification; session metadata.order_id must bind back = true double-bind), 0009 push insert policy requires a resolved profile (anon could insert user_id='' via coalesce). 003J lifecycle: getLaunchUrl latched once/session + navigateRef+[] deps in NativeAppBootstrap/NativeRuntime (launch-URL trap re-navigated to the launch route on EVERY nav, poisoning checkout return), push-listener churn + cancellation races fixed, persister unsubscribe captured. 003K admin parity: dual-ban (email+user_id) matching web onBanEmail, ban only offered w/ real user_email and never on removed posts, estimated_delivery written on ship (calcEstimatedDelivery/SHIPPING_METHODS ported), full refund payload (amount/reason/refunded_at) via native refund sheet incl. delivered orders, optional moderation_reason on hide/remove, mutateAsync so BanDialog's pending guard works, rlt_admin_log audit events (verified: write-only bus, no consumer on either surface). 003L routing/UX: NativeAuthFrame close-affordance on the 4 auth routes (dead-end fix), alias resolution centralized in NativePublicShell via nativeAliasFor (+?product=; inline dead-code implementations removed), news by-id fetch fallback, replies titled "Re: <thread>", missing session_id = soft "Order confirming" (deploy skew, not red), tab memory skips checkout/auth paths, notification taps host-checked via mapUrlToRoute, useWindowedList restoreKey re-seeds windows so scroll restore lands (forum+news). 003M honesty: admin attention badges now true head:true count queries (doc claimed "id-only projections" while fetching full rows — both fixed), NATIVE_ARCHITECTURE corrected (allowlist, DB-first double-bind description, web ?success=true clears cart unverified [pre-existing, deliberate], universal_links "fixed in 003J, device-unverified", ~71KB entry qualified with ~1.0MB raw/~320KB gz total preloaded web JS, push RLS admin-override note), @capacitor guard test also catches side-effect imports + export-from re-exports, AGENT_HANDOFF refreshed to RLT-IOS-003 reality, moderator-native gap + PUSH2 shared-device token reassignment documented as deferred (RLT-IOS-004). Plus ci(codemagic): auto-increment build number from latest TestFlight build (upload was failing on reused CFBundleVersion=1). Validation at 003M: tests 221/221, lint 0, typecheck 0, build 0, cap sync clean.
