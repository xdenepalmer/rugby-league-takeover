...
## ADMIN-CC — Admin "Owner Command Center" action queue (admin UX epic, step 2)
- Owner provided an admin/public UX audit. **Step 1 (stabilize WIP / build green): verified — current `main` (`c2a895a`) builds green (build 0, tests 45/45, lint 0); the reported Home.jsx bugs (duplicate HeroSection, missing `</LazySection>`) are NOT on main** (imported/used once; LazySection 7 open/7 close balanced) — they were transient uncommitted WIP. Nothing to fix on the canonical branch.
- **Step 2 (Owner Command Center):** new `src/components/admin/AdminActionQueue.jsx` — a "Needs you now" prioritised task queue at the top of `OverviewPanel`, derived purely from data the panel already fetches (no extra requests): orders to fulfil (status pending/paid/processing), posts to moderate (`is_published!==true`), low-stock products (`stock_quantity<=5`), new registrations (7d), unpublished content (news+testimonials drafts). Each row is a touch-friendly (≥52px) `Link` to the relevant `/admin/*` section; shows "All clear" when empty. Branch `feat/admin-command-center` off `c2a895a`.
- Validation: **45/45** · lint · typecheck · build green. Frontend-only (Publish only). 2 files.
- **Remaining (build order):** expand StoreOrder shipping/status model (delivery est, method, customer notes, timeline) + sync Admin/Account/Store language [needs entity deploy]; moderation inbox (bulk, soft-delete/restore, reasons, report queue); public "My Takeover" account dashboard; contextual mobile command sheets per page; notification preferences + templates. Plus the earlier ADMIN-UX inc.1 (PR #17 confirmations) + shared list primitives.
