...
## fix: clear ads-platform lint errors (MERGED, PR #11)
- Concurrent ads/sponsorship commit `8044c3e` left unused imports → main lint RED (3 errors). Removed `Monitor`/`Smartphone` (AdRevenueTracker.jsx) + `CheckCircle2` (SponsorManager.jsx). Squash-merged → main `c3a7679`. Lint green. 4th concurrent regression — recommend CI gate on auto-sync commits.

## RLT-002B — "Team you support" sourced from Team management (Claude)
- Owner request: the Register Interest team dropdown should list teams from team management, not a hardcoded list. Built on green `main` (`c3a7679`) via fixed-SHA worktree, branch `feat/interest-teams-from-management`.
- Frontend `TravelSection.jsx`: now fetches `base44.entities.Team.list("sort_order", 200)` (mirrors ProfileTab), builds the dropdown from **active managed teams + "Other"**, falling back to the built-in list only if none configured. (`useMemo`/`useQuery` added.)
- Validation relaxed for dynamic teams: `normalizeInterestRegistration` (public-forms.js) and `submitRegistration/entry.ts` now require a **non-empty, length-capped** `team_supported` instead of gating on the hardcoded `SUPPORTED_TEAMS` (removed the now-unused const in the function; kept the `public-forms` export for the admin filter). `InterestRegistration.jsonc` `team_supported` **enum removed** (free string) so admin-added teams are accepted.
- Tests: `tests/public-forms.test.mjs` +2 (accepts dynamic team name; requires non-empty team). **45/45** · lint · typecheck · build green.
- 🔴 **HARD deploy dependency (NOT graceful):** because the old `submitRegistration` + entity enum still gate on the hardcoded list, **do NOT publish the frontend without deploying `submitRegistration` + the `InterestRegistration` entity** — otherwise users picking a newly-added team get rejected (400). Single-agent deploy prompt provided. Explicit-path staging (no `git add -A`).
