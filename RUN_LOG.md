...
## RLT-SEC-001 — Security audit sweep (Devin)
- Scope: secrets/keys, injection, input validation, dependency CVEs, CORS, exposed endpoints, missing auth checks — across `src/**`, `supabase/functions/**`, `supabase/migrations/**`, CI and host config.
- **Fixed (critical/high):**
  1. `notifyProductRelease` ran with the service role for **any anonymous caller** — it could mail every release-list subscriber of any live product and burn their one-shot `notified_at` flag. Now admin-only, with an optional automation path (`RELEASE_NOTIFY_SECRET` + `x-task-secret`, constant-time compare).
  2. `forum_posts_view` (granted to `anon`) returned `user_email` verbatim → the email address of every member who has ever posted was readable with the publishable key. Migration `0018` masks it to admin/owner, matching `testimonials_view`. The account "my posts" filter still works (own rows keep the real value).
  3. Open-redirect in the post-auth `?next=` handling: `startsWith("/") && !startsWith("//")` misses `"/\evil.com"`, which browsers normalise to `//evil.com`. New `src/lib/safe-redirect.js` (`safeInternalPath`) is now used by Login, Register, `loginWithProvider` and `logout`.
  4. Dependency CVEs — `npm audit fix` (lockfile only, no range changes): postcss path traversal, vite `fs.deny` bypass, js-yaml + brace-expansion/minimatch + tar DoS, react-router-dom open redirect/XSS → 7.18.1.
  5. Information disclosure — 13 anonymous-reachable functions returned raw `(error as Error).message` (Postgres table/column/constraint text). New `serverError()` helper in `_shared/shared.ts` logs the cause and answers generically; admin-only functions keep their detailed errors.
- **Docs:** `.env.example` now documents `SHIPPING_QUOTE_SECRET` (required and fail-closed for non-pickup checkout), `RLT_APP_ID` and `RELEASE_NOTIFY_SECRET`.
- **New tests:** `tests/edge-function-security.test.mjs` (notifyProductRelease authz, no raw error echo, shared.ts sync, forum email masking), `tests/safe-redirect.test.mjs`.
- Verified-healthy: RLS/policies and sanitising views, Stripe webhook signature + amount/currency/app checks, signed shipping quotes (HMAC, constant-time, expiry, cart fingerprint), server-side price/stock authority, admin/owner checks in `adminUsers`/`inviteUser`/`cancelOrder`/`stripeRefund`/`forumAction`, no SQL string interpolation anywhere (PostgREST + one parameterised RPC), no `dangerouslySetInnerHTML` on user content, strict CSP/HSTS/Permissions-Policy in `vercel.json`, no debug endpoints, no hardcoded secrets (the publishable anon key ships by design).
- **Not fixed — reported (needs product decisions):** `recordAdEvent` is unauthenticated and unthrottled (ad impression/click inflation feeds `AdRevenueTracker`); `forumAction` `view`/`report` likewise (counter inflation, report dedups per IP); `forumAvatars` lets anyone enumerate the member list (opt-in fields only); `VITE_RAPIDAPI_KEY` is shipped in the client bundle by design (needs an Edge Function proxy to protect the quota); Edge Functions send `Access-Control-Allow-Origin: *` (bearer-token auth, no cookies, so no CSRF path — tightening it would break preview deploys and the Capacitor shell); react-router RSC advisory has no fixed release (RSC mode unused) and the remaining `brace-expansion` advisory is dev-only (ESLint) with no non-breaking fix.
- Validation: **npm test 190/190 · lint 0 · typecheck 0 · build 0.**
